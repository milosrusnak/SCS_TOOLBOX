...
if k == 1 :
    ...
    union = arcpy.Union_analysis (U_layer, "union", "ALL")
    ...
    with arcpy.da.UpdateCursor(union, fields_fam) as cursor:
        for row in cursor:
            ...
            cursor.updateRow(row)    
    name = "fam_layer.shp"
    union2 = arcpy.management.MultipartToSinglepart(union, "union2")
    ...
    Fpath_dis = arcpy.management.CopyFeatures(flow, "Fpath_dis")
    distance= cellsize*5
    arcpy.Densify_edit(Fpath_dis, "DISTANCE", distance)
    Fpath_point = arcpy.FeatureVerticesToPoints_management(Fpath_dis, "Fpath_point", "All")
    Fpath_point_Z = arcpy.sa.ExtractValuesToPoints(Fpath_point, dem, "Fpath_point_Z", "NONE", "VALUE_ONLY")
...
if k == 3:
    ...
    with arcpy.da.UpdateCursor(union, fields_fam) as cursor:
        for row in cursor:
            ...
            cursor.updateRow(row)    
    name = "fam_layer.shp"
    union2 = arcpy.management.MultipartToSinglepart(union, "unionFAM2")
    arcpy.DeleteField_management(union2, "ORIG_FID")
