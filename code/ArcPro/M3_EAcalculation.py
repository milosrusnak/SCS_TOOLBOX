...
Last update on 27 NOVEMBER 2024
@author: Milos Rusnak
...
def OrientationMask (polygon_old, polygon_young, centerline_old, centerline_young, year_old, year_young):
    ...
    Vars:\n
    \t polygon_old = channel polygon envelop (without hollows) for older date \n
    \t polygon_young = channel polygon envelop (without hollows) for younger date \n
    \t centerline_old = centerline for older date \n
    \t centerline_young = centerline for younger date \n
    \t year_old = info about date for old polygon \n
    \t year_young = info about date for young polygon \n
    ...
    bufLo = arcpy.analysis.Buffer(cenOlder2, "%ScratchWorkspace%\\bufLo", 1, "LEFT", "FLAT")
    ...
    bufRo = arcpy.analysis.Buffer(cenOlder2, "%ScratchWorkspace%\\bufRo", 1, "RIGHT", "FLAT")
    ...
    arcpy.management.AddField(bufLRclipo, "S_{}".format(y1), "TEXT")
    with arcpy.da.UpdateCursor(bufLRclipo, ("S_{}".format(y1), "SIDEL", "SIDER")) as cursor:
      for row in cursor:
         row[0] = row[1]+row[2]
         cursor.updateRow(row)
    ...
    fm.addTable(polcnto)
    keepers = ["S_{}".format(y1)]
    for field in fm.fields:
      if field.name not in keepers:
         fm.removeFieldMap(fm.findFieldMapIndex(field.name))
    sideMaskOlder = arcpy.analysis.SpatialJoin(polcnto, bufLRclipo, "%ScratchWorkspace%\\sideMaskOlder", "JOIN_ONE_TO_MANY", "",fm, "CONTAINS")
   
    #C Identification of side mask orientation YOUNG
    bufLy = arcpy.analysis.Buffer(cenYounger2, "%ScratchWorkspace%\\bufLy", 1, "LEFT", "FLAT")
    ...
    bufRy = arcpy.analysis.Buffer(cenYounger2, "%ScratchWorkspace%\\bufRy", 1, "RIGHT", "FLAT")
    ...
    arcpy.management.AddField(bufLRclipy, "S_{}".format(y2), "TEXT")
    with arcpy.da.UpdateCursor(bufLRclipy, ("S_{}".format(y2), "SIDEL", "SIDER")) as cursor:
      for row in cursor:
         row[0] = row[1]+row[2]
         cursor.updateRow(row)
    ...
    fm.addTable(polcnty)
    keepers = ["S_{}".format(y2)]
    for field in fm.fields:
      if field.name not in keepers:
         fm.removeFieldMap(fm.findFieldMapIndex(field.name))
    sideMaskYounger = arcpy.analysis.SpatialJoin(polcnty, bufLRclipy, "%ScratchWorkspace%\\sideMaskYounger", "JOIN_ONE_TO_MANY", "",fm, "CONTAINS")
...
for fclist in channel_layer:
   ...
   with arcpy.da.SearchCursor(fclist, field_check) as cursor:
      for row in cursor:
        d = row [0]
        da = datetime.date.isoformat(d)
        date = str(da[:4])+str(da[5:7])+str(da[8:10]) 
   newName = "CH_"+ str(date) + ".shp"
...
for fclist in centerline_layer:
   ...
   with arcpy.da.SearchCursor(fclist, field_check) as cursor:
      for row in cursor:
        ...
        da = datetime.date.isoformat(d)
        date = str(da[:4])+str(da[5:7])+str(da[8:10]) 
   newName = "centro_"+ str(date) + ".shp"
   year_check.append(date)
   date_check.append(d)
   newNamepath = os.path.join(ws, newName)
   if arcpy.Exists(newNamepath):
        arcpy.AddMessage("{} exists, not copying".format(newNamepath))
...
date_txt_sort = sorted(date_txt)
date_check_sort = sorted(date_check)
...
for i in range(len(EA_layer_sort)):
   arcpy.management.AddField(EA_layer_sort[i], "y_{}".format(year_sort[i]), "TEXT")
...
for i in range(len(EA_island)-1):
   ...
   arcpy.AddMessage("STEP 5 Calculate in-channel proces of erosion and deposition for dates {} and {}".format(input5, input6))
   ...
   y2 = input6
   d1 = date_txt_sort[i]
   d2 = date_txt_sort[i+1]
   name = "EA_processes{}_{}.shp".format(y1,y2)

   fld = ["y_{}".format(y1), "T_{}".format(y1), "y_{}".format(y2), "T_{}".format(y2)]
   ...
   fld1 = ["EA", "S_{}".format(y1), "S_{}".format(y2),"y_{}".format(y1), "y_{}".format(y2)]
   ...
   arcpy.management.AddField(unionEAmask, "D1", "DATE")
   with arcpy.da.UpdateCursor(unionEAmask, "D1") as cursor:
      for row in cursor:
         row[0] = d1
         cursor.updateRow(row)

   arcpy.management.AddField(unionEAmask, "D2", "DATE")
   with arcpy.da.UpdateCursor(unionEAmask, "D2") as cursor:
      for row in cursor:
         row[0] = d2
         cursor.updateRow(row)

   arcpy.management.AddField (unionEAmask, "span_days", "LONG")
   with arcpy.da.UpdateCursor(unionEAmask, ("span_days", "D1", "D2")) as cursor:
      for row in cursor:  
         delta = row[2] - row[1]
         row[0] = delta.days
         cursor.updateRow(row)
   
   arcpy.management.AddField (unionEAmask, "span_years", "LONG")
   with arcpy.da.UpdateCursor(unionEAmask, ("span_years", "span_days")) as cursor:
      for row in cursor:  
         row[0] = row[1]/365
         cursor.updateRow(row)      

   fld3 = ["y_{}".format(y1), "T_{}".format(y1), "y_{}".format(y2), "T_{}".format(y2), "EA", "direction", "period", "span_days", "span_years", "migration"]
   ...
   unionEAdiss = arcpy.management.Dissolve(unionEAmask, "%ScratchWorkspace%\\unionEAdiss", ["EA", "direction", "period", "span_days", "span_years", "migration"])
...
if len(statistics) != 0:
    ...
    for i in range(len(EAprocess)):
      ...
      diss = arcpy.management.Dissolve(EAprocess[i], "%ScratchWorkspace%\\diss", ["span_days", "migration", "period", "span_years"])
      ...
      fld4 = ["migration", "span_days", "EA_rate_A", "EA_rate_m"]
